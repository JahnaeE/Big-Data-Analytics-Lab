library(dplyr)
library(tidyr)
library(ggplot2)
library(tidygeocoder)
library(maps)
library(reshape2)
library(purrr)
library(ggrepel)
library(usmap)
library(ggspatial)
library(sqldf)
library(tigris)
library(plotly)
library(htmlwidgets)

#combined_results <- read.csv("combined.csv")
#stance detection results
setwd("C:/Users/13212/OneDrive/Documents/College/USF/GA Position/Dominon/Stance")
combined_results <- read.csv("complete_stance.csv")
combined_location_names <- data.frame(combined_results$Date, combined_results$Location.Name, combined_results$stance)

combined_location_names <- combined_location_names %>%
  drop_na(combined_results.Location.Name)

colnames(combined_location_names) <- c("Date", "Location_Name", "Stance")

#locationCounts <- sqldf("SELECT Location_Name, COUNT(Location_Name) FROM combined_location_names 
# GROUP BY Location_Name ORDER BY COUNT(Location_Name) DESC")


#now including the top stance
#the stance with the highest count represents that loactions overall stance
locationCounts <- combined_location_names %>%
  filter(Location_Name != "") %>%  # Exclude empty locations if needed
  group_by(Location_Name) %>%
  summarise(
    location_count = n(),
    in_favor_count = sum(Stance == "in favor"),
    against_count = sum(Stance == "against"),
    neutral_count = sum(Stance == "neutral or unsure"),
    representing_stance = case_when(
      in_favor_count >= against_count & in_favor_count >= neutral_count ~ "in favor",
      against_count >= in_favor_count & against_count >= neutral_count ~ "against",
      TRUE ~ "neutral or unsure"
    )
  ) %>%
  arrange(desc(location_count))


#combined_GeoCodes <- locationCounts %>%
#  geocode(address = Location_Name,
#          method = "arcgis")

combined_GeoCodes <- locationCounts %>%
  select(Location_Name, location_count, representing_stance) %>%
  geocode(address = Location_Name,
          method = "arcgis")


#combined_GeoCodes_Adresses <- combined_GeoCodes %>%
#  reverse_geocode(lat = lat, long = long, method = 'arcgis',
#                  address = address_found, full_results = TRUE)

combined_GeoCodes_Adresses <- combined_GeoCodes %>%
  reverse_geocode(lat = lat, long = long, method = 'arcgis',
                  address = address_found, full_results = TRUE) %>%
  left_join(locationCounts, by = c("Location_Name" = "Location_Name"))



#colnames(combined_GeoCodes_Adresses)[2] <- ("Location_Counts")

#GeoCodes_USA <- combined_GeoCodes_Adresses %>%
#  filter(CountryCode == "USA") %>%
#  filter(Region != "Alaska") %>%
#  filter(Region != "Hawaii")


#World Map
world_coordinates <- map_data("world")


#ggplot(data = combined_GeoCodes, aes(long, lat)) +
#  geom_map(data = world_coordinates, map = world_coordinates, 
#           aes(long, lat, map_id = region), 
#           color = "white", fill = "grey", size = 0.2) +
#  geom_point(alpha = 0.5) +
#  labs(title = "BrandWatch Location Data - World View")

#updated colors to represent the stance for that loaction
ggplot(data = combined_GeoCodes, aes(long, lat, fill = representing_stance)) +
  geom_map(data = world_coordinates, map = world_coordinates, 
           aes(long, lat, map_id = region), 
           color = "white", fill = "grey", size = 0.2) +
  geom_point(alpha = 0.5, shape = 21, size = 2) +
  labs(title = "BrandWatch Location Data - World View") +
  scale_fill_manual(values = c("in favor" = "green", "against" = "red", "neutral or unsure" = "blue"))


#USA Map
ggplot(GeoCodes_USA, aes(long, lat)) +
  borders("state") +
  geom_point(aes(size = Location_Counts), color = "red", alpha = 0.7) +
  geom_label_repel(data = GeoCodes_USA %>%
                     filter(Location_Counts > 50),
                   aes(label = Location_Name), max.overlaps = Inf, box.padding = 1.8, segment.size = 1.0, alpha = 0.7) +
  labs(title = "BrandWatch Location data within the US", caption = "Only Locations with over 50 mentions are labeled") +
  theme_minimal()


#plotly
#us map
# geo styling
library(plotly)

#deleting usa row since its an outlier and we know that we are in the us
GeoCodes_USA <- GeoCodes_USA[-1, ]
g <- list(
  scope = 'usa',
  projection = list(type = 'albers usa'),
  showland = TRUE,
  landcolor = toRGB("gray95"),
  subunitcolor = toRGB("gray85"),
  countrycolor = toRGB("gray85"),
  countrywidth = 0.5,
  subunitwidth = 0.5
)

fig <- plot_geo(GeoCodes_USA, lat = ~lat, lon = ~long) 

fig <- fig %>% add_markers(
  text = ~paste(Location_Name, "Count:", Location_Counts),
  size = ~Location_Counts, color = I("blue"), symbol = I("circle"),  # Change color to blue
  opacity = 0.7, hoverinfo = "text"
)

fig <- fig %>% layout(
  title = 'BrandWatch Location data within the US',
  geo = g,
  showlegend = FALSE  
)

# Display the interactive plot
fig

saveWidget(fig, file = "bd_mapUS.html")

#world map
library(plotly)


world_coordinates <- map_data("world")

world_map <- plot_geo(world_coordinates, locationmode = 'ISO-3') 

world_map <- world_map %>% add_markers(
  data = combined_GeoCodes,
  x = ~long, y = ~lat,
  text = ~paste(Location_Name, "<br />Count: ", `COUNT(Location_Name)`),
  size = ~`COUNT(Location_Name)`, color = I("red"), opacity = 0.7, symbol = I("circle")
)

world_map <- world_map %>% layout(
  title = 'BrandWatch Location Data - World View',
  geo = list(
    showland = TRUE,
    landcolor = toRGB("gray95"),
    subunitcolor = toRGB("gray85"),
    countrycolor = toRGB("gray85"),
    countrywidth = 0.5,
    subunitwidth = 0.5
  ),
  showlegend = FALSE 
)

world_map

saveWidget(world_map, file = "bd_mapworld.html")

#updated plotly world map with stance
world_coordinates <- map_data("world")

# Create a basic geographic plot using plot_geo
world_map <- plot_geo(combined_GeoCodes, locationmode = 'ISO-3') 

# Add markers for the combined_GeoCodes dataset
world_map <- world_map %>% add_markers(
  x = ~long, y = ~lat,
  text = ~paste(Location_Name, "<br />Count: ", location_count),
  size = 0.5, color = ~representing_stance,
  colors = c("in favor" = "green", "against" = "red", "neutral or unsure" = "blue"),
  opacity = 0.7, symbol = I("circle")
)

# Set the layout of the plot, including the title and geographic configuration
world_map <- world_map %>% layout(
  title = 'BrandWatch Location Data - World View',
  geo = list(
    showland = TRUE,
    landcolor = toRGB("gray95"),
    subunitcolor = toRGB("gray85"),
    countrycolor = toRGB("gray85"),
    countrywidth = 0.5,
    subunitwidth = 0.5
  )
)

# Display the interactive plot
world_map
